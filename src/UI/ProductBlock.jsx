  ...
  return (
    <div className="relative flex flex-col items-center bg-white p-4 rounded-lg shadow-lg hover:shadow-2xl hover:bg-gray-100 hover:scale-95 transition-all duration-300 h-full space-y-4">
      ...
      <div className="flex items-center justify-center mb-4">
        {newPrice ? (
          <>
            <span className="text-gray-500 text-xs line-through mr-2">{price.toLocaleString()} VNĐ</span>
            <span className="text-red-500 font-bold text-sm">{newPrice.toLocaleString()} VNĐ</span>
          </>
        ) : (
          <span className="text-gray-700 font-bold text-sm">{price.toLocaleString()} VNĐ</span>
        )}
      </div>

      <div className="flex items-center justify-between w-full space-x-4">
        <button onClick={handleAddToCart} className="w-32 bg-yellow-400 text-white py-1 rounded-md hover:bg-red-500 transition duration-200">
          Thêm Giỏ Hàng
        </button>
        <button onClick={handleAddToWishlist} className="h-8 px-2 py-1 bg-gray-200 rounded hover:text-orange-400 rounded-lg">
          <FontAwesomeIcon icon={faHeart} />
        </button>
      </div>
    </div>
